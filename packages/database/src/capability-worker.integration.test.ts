...
describe.skipIf(!databaseUrl)('restricted capability staging worker',()=>{
 it('stages exact snapshots with bounded retries, readback, stale denial and restricted audit',async()=>{
  ...
  try{
   ...
   const events=(await admin.query("SELECT actor_service_id,actor_user_id,after_state FROM identity_audit_events WHERE organization_id=$1 AND actor_type='service' AND actor_service_id='capability-worker'",[org.organizationId])).rows;
