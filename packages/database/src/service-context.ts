...
async function assertServiceConnection(client: Client): Promise<void> {
  ...
  const result = await client.query<{ allowed: boolean }>(
    `SELECT pg_has_role(current_user, 'company_human_service', 'member')
      ...
      AND (SELECT c.relowner <> r.oid FROM pg_class AS c WHERE c.oid = 'public.organizations'::regclass) AS allowed
...
export async function setInvitationActorContext(client: Client, actorUserId: string, invitationHash: string): Promise<string> {
  await assertServiceConnection(client);
  const actor = UserIdSchema.parse(actorUserId);
  if (!/^[0-9a-f]{64}$/.test(invitationHash)) throw new Error("Invalid invitation hash");
  await client.query("SELECT set_config('company_human.user_id', $1, true), set_config('company_human.invitation_hash', $2, true)",
    [actor, invitationHash]);
