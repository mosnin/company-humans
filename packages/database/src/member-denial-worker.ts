...
export async function finishMemberDenial(url:string,organizationId:OrganizationId,lease:MemberDenialLease,raw:Receipt,accessReceipt?:MemberAccessCommand):Promise<void> {
  ...
  await transaction(url,organizationId,async client=>{
    const result=await client.query<Row>(`SELECT j.*,c.operation,c.idempotency_key,c.source_policy,pm.product_instance_id,pm.membership_id,pm.external_member_id,
    ...
    const missingFence=row.source_policy!==null&&receipt.status==='succeeded'&&(!lease.accessCommand||!fencedReceipt);
