...
import { OrganizationIdSchema, UserIdSchema, ProductInstanceIdSchema, MembershipIdSchema, TeamIdSchema } from "@company-human/contracts";
...
export interface UsageWindow {
  ...
  from: string;
  until: string;
  breakdown: "organization" | "product" | "instance" | "team" | "member";
...
function validateWindow(window: UsageWindow) {
  ...
  if (!["test","production"].includes(window.environment)
    || !["organization","product","instance","team","member"].includes(window.breakdown)) throw new Error("Invalid usage scope");
}
...
export async function aggregateUsageInTransaction(client: Client, userId: string, window: UsageWindow): Promise<UsageAggregate[]> {
  ...
  const result = await client.query<UsageAggregate>(`WITH scoped AS (
    SELECT e.product_id,e.meter_key,e.meter_version,e.unit,m.aggregation,e.quantity,e.occurred_at,e.reported_at,e.event_id,
  ...
  ) SELECT $1::text AS "organizationId",product_id AS "productId",instance_id AS "productInstanceId",
    member_id AS "membershipId",attributed_team_id AS "teamId",$2::text AS environment,
    ...
    FROM ranked GROUP BY product_id,meter_key,meter_version,unit,aggregation,instance_id,member_id,attributed_team_id
    ORDER BY product_id,meter_key,meter_version,unit,instance_id NULLS FIRST,member_id NULLS FIRST,attributed_team_id NULLS FIRST`,
  [window.organizationId,window.environment,window.from,window.until,window.breakdown,window.productInstanceId??null,window.membershipId??null,window.teamId??null]);
  return result.rows;
}
