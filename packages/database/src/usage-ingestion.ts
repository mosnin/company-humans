...
export async function ingestUsageEvent(client: Client, input: unknown, authority: UsageSigningAuthority): Promise<{ eventId: string; disposition: "accepted" | "quarantined"; duplicate: boolean }> {
  ...
  try {
    ...
    const existing = await client.query<{event_id:string;disposition:"accepted"|"quarantined";identical:boolean}>(`SELECT event_id,disposition,envelope=$1::jsonb AS identical FROM public.usage_events
      WHERE event_id=$2 OR (source_system=$3 AND source_event_id=$4) OR idempotency_key=$5`, [JSON.stringify(body),event.eventId,event.source.system,event.source.eventId,event.idempotencyKey]);
