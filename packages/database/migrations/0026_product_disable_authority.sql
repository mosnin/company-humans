-- Disable preserves observed provider state; runtime credentials cannot fabricate
-- success or re-enable a disabled instance without the future reconciliation boundary.
DROP POLICY product_update_authority ON public.product_instances;
...
BEGIN
  ...
  allowed := CASE TG_TABLE_NAME
    ...
    WHEN 'product_instances' THEN ARRAY['desired_enabled','updated_at']
    ELSE ARRAY[]::text[] END;
