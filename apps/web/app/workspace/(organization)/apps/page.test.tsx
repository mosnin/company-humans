...
it('shows assignment states without claiming launch authority',async()=>{mocks.apps.mockResolvedValue(['preparing','suspended','unavailable','access_check_required'].map((status,id)=>({id:String(id),name:'Scalar',instanceKey:'primary',status})));const html=renderToStaticMarkup(await Page());for(const label of ['Being prepared','Access paused','Currently unavailable','Access needs verification'])expect(html).toContain(label);expect(html).not.toContain('<a');});
