...
export default async function AuditPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <><PageHeader title="Audit history" description="Recorded changes to your organization, people, teams, and permissions." />
    <Card><CardContent>{!data.events.length ? <p className="t-body text-ink-2">No events on this page.</p> : <ol className="divide-y divide-border">{data.events.map(event=><li key={event.id} className="py-4 first:pt-0 last:pb-0">
      ...
      <p className="mt-1 t-body text-ink-2">By {event.actorName} · {event.actorType}</p>
      <details className="mt-3"><summary className="t-body cursor-pointer text-ink-2">View change details</summary><dl className="mt-3 space-y-2 break-all t-caption"><dt className="font-medium">Actor ID</dt><dd>{event.actorUserId ?? event.actorServiceId}</dd><dt className="font-medium">Target</dt><dd>{event.targetType}: {event.targetId}</dd></dl>
