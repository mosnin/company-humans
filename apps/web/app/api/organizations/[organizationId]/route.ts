...
import { renameOrganization } from "@company-human/database/organization-authority";
import { resolveAuthenticatedUser } from "@/lib/authenticated-user";

export const runtime = "nodejs";

export async function PATCH(request: NextRequest, context: { params: Promise<{ organizationId: string }> }): Promise<NextResponse> {
  ...
  if (typeof name !== "string") return NextResponse.json({ error: "Invalid request" }, { status: 400 });
  const { organizationId } = await context.params;
  try {
    await renameOrganization(databaseUrl, { actorUserId: identity.userId, organizationId, name });
    return NextResponse.json({ status: "ok" }, { headers: { "cache-control": "no-store" } });
  } catch {
    return NextResponse.json({ error: "Organization change denied" }, { status: 403 });
  }
}
