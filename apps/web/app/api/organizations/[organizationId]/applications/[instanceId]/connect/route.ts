...
import { z } from "zod";
const Body = z.object({ externalOrganizationId: z.string().trim().min(1).max(256) }).strict();
...
export async function POST(request: NextRequest, context: { params: Promise<{ organizationId: string; instanceId: string }> }): Promise<NextResponse> {
  ...
  if (!body.success) return NextResponse.json({ error: "Invalid connection request" }, { status: 400 });
  ...
  try {
    const operationId = await requestProductConnection(databaseUrl, { ...body.data, actorUserId: identity.userId, organizationId: organizationId.data, productInstanceId: instanceId.data });
    return NextResponse.json({ operationId, providerConnectionConfirmed: false }, { status: 202, headers: { "cache-control": "no-store" } });
  } catch (error) {
    if (error instanceof Error && error.message === "Connection target cannot change during setup") return NextResponse.json({ error: "A different organization was already requested. Refresh to review connection progress." }, { status: 409 });
    if (error instanceof Error && ["Application administration denied", "Product connection unavailable"].includes(error.message)) {
      return NextResponse.json({ error: "Application unavailable or permission denied" }, { status: 403 });
    }
    return NextResponse.json({ error: "Could not request connection. Please retry." }, { status: 503 });
  }
}
