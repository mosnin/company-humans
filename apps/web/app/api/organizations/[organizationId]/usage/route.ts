...
import { z } from "zod";
import { OrganizationIdSchema, ProductInstanceIdSchema, MembershipIdSchema, TeamIdSchema } from "@company-human/contracts";
...
const Query = z.object({
  ...
  from: Timestamp,
  until: Timestamp,
  breakdown: z.enum(["organization", "product", "instance", "team", "member"]),
  productInstanceId: ProductInstanceIdSchema.optional(),
  membershipId: MembershipIdSchema.optional(),
  teamId: TeamIdSchema.optional(),
}).strict().refine(value => Date.parse(value.from) < Date.parse(value.until), "Invalid usage window");
