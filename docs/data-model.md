...
The V2 usage-limit schema adds a required meter version to its shared policy and provider readback types. It adds no database field or migration. Existing `product_usage_limits`, revision rows and attempt receipts continue using V1 semantics; a future additive storage and worker migration must carry meter version without reinterpreting historical rows.
