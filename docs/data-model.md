...
The typed `ch_res_` reservation ID and V1 intent/projection/transition/reconciliation schemas are contracts only. No reservation or operation-to-usage mapping table exists. A future storage migration must independently enforce tenant and environment scoped source-operation uniqueness, transport-key conflict detection, contiguous append-only transitions, unique usage-event settlement, authoritative expiry, and atomic accounting across all applicable windows and scopes. The schema's `database_transaction` clock literal does not itself prove a database transaction occurred.
