...
The Company Human Postgres schema has global users, organizations, memberships, teams, roles, invitations, identity audit records, a draft product catalog, product instances, product-member mappings, desired entitlements, usage events, and versioned budget configuration. Tenant rows carry organization identity. Commercial billing, attribution, commission and payout ledgers remain unimplemented. Company OS Convex IDs are never Company Human canonical IDs.
...
Migration 0077 adds `workspace_module_revisions` with `(organization_id, module_key, revision)` identity, an eight-key native-module constraint, `enabled`, actor and database time. Rows are append-only and latest revision determines the effective organization setting. No row means revision zero with Work on and the other seven native modules off. A setting does not create a product instance, membership, entitlement, financial object or route permission. Direct SQL inserts and repository writes both create an identity audit event in the same transaction.
...
Migration 0068 adds private active and closed team-assignment intervals captured atomically from `team_memberships` writes. A single cutover instant seeds only assignments active when the migration takes its exclusive snapshot lock; ended assignments and unknown earlier gaps are not invented. New inserts, role changes, removal, reactivation and deletion close/open half-open intervals with tenant, team, membership, role and origin retained. Closed history retains canonical IDs without cascading foreign keys so later tenant cleanup cannot erase attribution history. Only a non-login trigger owner can append it; application, service and ingestion roles receive no direct table access. The existing mutable `team_memberships` row remains the operational assignment source. Migration 0069 checks named human usage against these intervals.
...
### Temporal team attribution for usage

Migration 0068 records team-assignment intervals from its cutover onward. Migration 0069 checks a signed human usage event with a non-null team against those intervals at `occurred_at`, including delayed events in a closed interval. An event before the cutover, in a gap, for another team, or in the future has no verified team attribution and is rejected. The same check runs before a quarantined human event is released. An exact stored event remains an idempotent retry after an assignment ends. Null-team and non-human events do not establish team-budget eligibility.

Migration 0070 stores `budget_policies` as immutable tenant, product, exact meter version/unit, UTC window and typed scope identities. Composite references bind product instances, teams and members to the policy tenant; the exact meter tuple references the immutable registry. Several policies may share a scope and window so warning and stop thresholds coexist. `budget_policy_revisions` append exact finite quantities, action, active/disabled status, actor and database time. Latest revisions form the current configuration; historical rows are never rewritten. This is desired policy data, not a consumption counter or provider receipt.

Migration 0071 adds a nullable, stored generated `usage_events.capability_key` projected directly from `envelope.payload.capabilityKey`. The signed usage contract accepts a canonical key or null and maps legacy omission to null. The generated field cannot be independently inserted or rewritten; a format constraint rejects invalid non-null values, and an organization/product/key/time index supports scoped reads. The runtime reader can select the projected key without access to the private envelope or signature. Unknown historical attribution remains null. Capability aggregation is a separate breakdown; organization, product, instance, team and member totals continue to combine all capabilities. Reported attribution does not prove catalog declaration, member authorization or provider enforcement.

Migration 0072 adds no budget or usage data table. It creates a no-login snapshot role with column-level grants and tenant RLS for current policies, revisions, catalog bindings and projected usage. The snapshot reader treats zero as measured absence only after a complete authorized read for each applicable scope and window. It writes no consumption, reservation or billing record.

The typed `ch_res_` reservation ID and V1 intent/projection/transition/reconciliation schemas began as contracts only. Migration 0075 adds inaccessible reservation tables with relational source-operation and transport-key uniqueness, ordered append-only transitions, unique usage-event binding and database-stamped expiry. It does not validate every serialized V1 field or create a trusted event-to-reservation mapping or writer. A future restricted writer must verify the full current policy set, signed event, team attribution and atomic accounting across all applicable windows and scopes. The schema's `database_transaction` clock literal alone does not prove a database transaction occurred.

Migration 0074 adds a nullable `usage_events.source_operation_id` generated from `envelope.source.operationId`. The V1 signed event source accepts that bounded optional field; legacy signed envelopes omit it and project SQL null without rewriting history. An operation may emit multiple usage events, so the projection has a nonunique tenant/product/environment/source lookup index. The column is not an independent mutable identity and is not a reservation settlement record.

Migration 0075 adds `budget_reservation_intents` and `budget_reservation_ledger` as **inaccessible schema scaffolding**. Intent rows have distinct tenant/product/environment uniqueness for source operations and transport retry keys, exact sum meter/instance/member/team references, finite positive quantities and database-stamped expiry. The ledger has typed ordered transitions and a global unique usage event ID; a later usage event may append after settled, released or expired without changing that capacity state. Both tables are append-only under forced RLS with no policies or runtime grants. Serialized request, projection, fingerprint, provenance and admin fixture event data are untrusted candidates: relational storage is not complete V1 validation, verified accounting or provider proof.

### Human Work obligation and report

Migration 0078 adds `human_assignments` and `human_assignment_completions`. Each carries organization identity and composite tenant references. An assignment records its assignee, optional team, creating human, title, objective, due time, priority, expected outcome, evidence requirement, and `manager` source. A completion records the exact assignee's reported outcome and optional or required evidence. One completion per assignment and immutable row triggers preserve the original report. Audit inserts occur in the same transaction as each fact. A completion is a contributor claim, not a verified business outcome. The first read API pages assignments in batches of 50 by offset; concurrent changes can shift later pages.
