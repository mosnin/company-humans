...
### Stage database HMAC verification without enabling it — 2026-09-23

Notion requires signed, scoped, idempotent and immutable usage, but does not prescribe where HMAC verification runs. Database verification is an implementation choice to close the direct-SQL bypass. Migration 0076 creates private, operator-managed key records and an ungranted function that computes HMAC over the exact JavaScript canonical UTF-8 body supplied by the caller; PostgreSQL `jsonb::text` is not used as HMAC input because its encoding differs. Keys are not seeded, and the current API/direct INSERT behavior remains. The function is unavailable to runtime roles until full signed-schema validation, duplicate and race handling, and cryptographic comparison are independently proven. A stolen source signing key or privileged database owner remains a separate authority risk after a future cutover.

### Default native modules separately from connected products — 2026-09-23

Notion 07 requires organization-controlled first-party modules and separately provisioned connected products. Migration 0077 gives every organization an effective Work setting of on and defaults CRM, Referrals, Earnings, Leaderboard, Team, Context and Creator off until explicitly configured. These are visibility preferences, not permissions or product entitlements. The first Work route must enforce both module availability and existing assignment permissions; enabling another module before its route exists must not add a dead navigation item. This choice should be revisited with actual contributor onboarding evidence.

### Start Human Work with reported manager assignments — 2026-09-23

Notion 07 and 08 make Company Human the authority for human obligations while Operate keeps agent tasks. The first native slice accepts only manager-originated assignments and one immutable assignee-reported completion; the UI labels this “Reported complete” because it does not verify a customer, CRM, or payment outcome. The Work page groups due items by UTC date until organization time zone configuration exists. Assignment pagination uses a bounded offset page and can shift under concurrent writes; cursor pagination and searchable assignable members are later scale gates. The remaining source types, related objects, blocked/canceled states, evidence review, and external outcome verification must be added before the full Human Work doctrine is complete.
