...
The pure budget quantity evaluator validates exact tenant/product/meter identity, expected production or test environment, meter aggregation, current UTC period, scope coverage and decimal comparisons. These are input checks, not provenance: it has no database credential, lock, reservation or provider call. The application must construct every quantity and the expected context from one restricted, tenant-authorized database snapshot before using results in entitlement logic. Current usage events have no capability key, so capability-scoped quantities cannot be proven from the event ledger. Missing or unverifiable attribution must fail closed in any future authorization path.
