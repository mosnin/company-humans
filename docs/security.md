...
The usage ingestion API verifies HMAC and authority scope before insert, but its database login currently has direct INSERT on `usage_events`, with RLS scope set by caller-controlled transaction settings. A holder of that database credential could bypass the API and create an `accepted` row without a valid signature. The shared event and audit signature helpers now verify the exact raw body before schema parsing; nested source, actor and audit target fields reject unknown data. The read-only stored-usage verifier rejects a fake accepted row when given an active trusted key and compares the signed projections, including the generated human actor ID. It uses the existing revalidator role in a read-only transaction; that credential can append quarantine release receipts outside this verifier, so receipt existence is not independent authorization proof. The verifier is not connected to a financial or provider decision. Before settlement or billing uses a stored event, remove the direct insertion bypass through a replacement ingestion boundary and prove key lifecycle, release authorization, team attribution and atomic reservation accounting.

Migration 0076 stages a private scoped signing-key registry and a database HMAC function. Its key table is empty outside fixtures, the function has no runtime EXECUTE grant, and the existing direct usage INSERT remains. The migration rejects an unexpected preexisting writer role with elevated attributes or membership edges, and rejects `pgcrypto` installed outside the expected schema. On PostgreSQL 16+, an unavoidable ADMIN-only edge may remain to the migration owner; the migration permits only that exact edge when it grants neither `SET` nor inherited access. This is locked-down preparation, not a completed ingestion cutover. Before any grant, the function needs strict schema equivalence, complete duplicate handling, concurrent retry tests, timestamp and metadata edge cases, and an independently reviewed digest comparison. The server route still uses the earlier ingestion path.
...
Migration 0068 introduces restricted team-assignment history for future temporal verification. Its trigger runs under a non-login writer with a fixed search path and table-specific grants; runtime roles cannot read or rewrite private intervals. Existing active rows begin at a conservative migration baseline. This storage closes future history loss but does not yet reject forged or omitted team fields in signed usage, so the team-budget bypass remains until ingestion enforcement is verified.
...
Migration0064 restricts new product-membership inserts to the current tenant, current administrator, active sponsor and target role grants, including `product.use` and registered catalog requirements. The eligibility helper runs with invoker rights; web and worker roles cannot execute it. A narrow, non-login identity owner holds the actor user share lock. The insert trigger locks catalog, tenant authorization and organization status before sorted actor and target parent locks. A direct service insert that waits for a catalog, grant, organization or actor edit must check the committed new truth. The admin list uses the same predicate but does not authorize a provider grant.

0065 narrows actor-context locking to inserts carrying the trusted service actor setting. This permits privileged migration repair of historical rows under a hosted migrator that is also a member of the service role. The restricted service role still cannot insert without actor context because the existing permissive and restrictive RLS policies reject it.
...
Migration 0070 gives the restricted service role SELECT and column-limited INSERT on budget policy identities and revisions under `budgets.manage` RLS and actor context. It grants only the meter registry columns needed for exact matching. Composite tenant references, immutable rewrite triggers and a serial revision trigger guard direct SQL. A narrow no-login guard locks catalog, authorization, organization, actor membership and actor identity before a fresh permission check; both direct SQL and the application writer use that order before the policy revision lock. Higher-isolation mutation transactions fail closed. Two-client tests prove both permission-revocation commit orders and revoke-first catalog, membership and user changes. Active revisions require a ready product with a declared meter and, for capability scope, a declared capability. Disabled revisions preserve shutdown history even if the catalog later retires. The API binds tenant and actor from the session, rejects cross-origin edits, and returns redacted errors. These settings do not authorize product access.

The pure budget quantity evaluator validates exact tenant/product/meter identity, expected production or test environment, meter aggregation, current UTC period, scope coverage and decimal comparisons. These are input checks, not provenance: it has no database credential, lock, reservation or provider call. The application must construct every quantity and the expected context from one restricted, tenant-authorized database snapshot before using results in entitlement logic. Migration 0071 adds a signed, reported capability key projected by the database from the immutable usage envelope. The runtime role reads only the generated key; envelope and signature stay private. The key is attribution, not catalog or permission verification. Legacy null attribution, unverified catalog bindings and missing provider operation binding must fail closed in any future capability budget authorization path.

Migration 0072's budget snapshot role is no-login, no-inherit, non-superuser and cannot bypass RLS. Its grants exclude event envelopes, signatures, provider identifiers and writes. Every tenant row requires both `budgets.manage` and `usage.read.all` for the actor in the selected organization. The diagnostic rejects privileged or table-owning session logins and runs in one repeatable-read, read-only transaction. This is an internal read boundary: a future caller must still authenticate the actor and bind a real provider operation before a budget decision can be trusted.

Migration 0073 removes six unused projected fields from that role: catalog access revision, membership user ID, team role, and product-membership ID plus its access and desired revisions. The reader selects only the columns it uses. This narrows exposure without changing budget evaluation or granting any write capability.

The reservation contract accepts caller-supplied projected quantities and structurally matching usage evidence, so neither is a trusted authorization source. Before any writer can reserve or settle, a restricted database boundary must bind the real operation, current policy revisions, accepted/released signed usage, outstanding capacity, tenant identity and source-event provenance in one serialized transaction. The optional signed operation ID proves what the scoped signer reported, not that the provider performed or authorized the operation; a free-standing evidence object cannot fill that gap. Contract results always deny an authorization or provider-enforcement claim.

Migration 0074 projects an optional source operation ID only from the immutable signed usage envelope; ordinary callers cannot supply a separate shadow value. Changing it after signing fails verification, and an exact retry with a changed signed body conflicts. The index is nonunique because one operation can emit several events. This is signer-reported provenance; a future settlement path still needs scoped key authority, accepted/released event verification and a unique transactional event-to-reservation binding.

Migration 0075 intentionally grants no runtime role any reservation-table access and creates no RLS policy or callable writer. Both tables force RLS; update/delete are rejected; owner fixture rows are not a trust boundary. The SQL guard narrows relational identity and finite quantity checks but does not cryptographically verify signature, validate every JSON constraint or establish current policy completeness. No consumer may use these tables until complete V1 validation, scoped signed-event verification, team assignment, race tests, least-privilege writer design and independent review pass.

### Human Work tenant and team boundary

Migration 0078 forces RLS on assignments and completion reports. The application read role sees only its own active-member assignments; the service role sees manager team or organization scope according to current grants and active team responsibility. Insert triggers recheck active actors and assignees, Work module state, team membership and assignment permissions after shared transaction locks. Only the exact active assignee can report completion, and a required evidence field cannot be omitted. Browser mutations require same-origin requests and server-resolved identity. The audit and assignment records cannot be rewritten by runtime roles. Candidate-member listing is currently unpaged; large-organization search and pagination remain to be built.

The Workspace settings page checks `organization.manage` before reading module state. Its Work PUT endpoint rejects cross-origin requests, resolves the actor server-side, accepts only the Work key and a strict revision body, and delegates the write to the existing audited service boundary. The database repeats current organization authority and serializes revisions. The seven unfinished native modules cannot be enabled through this endpoint; connected product access is governed separately.

Home reads only the active member's own Work assignments through the application role and forced-RLS tables, after checking the Work setting and `assignments.read.own`. The app summary uses the own-member reader and does not expose another member's app records or imply provider access. Disabled Work, unavailable configuration, an empty queue, and failed reads are displayed separately.
