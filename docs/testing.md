...
Migration 0070 applied and seeded on a fresh disposable local PostgreSQL database. The direct SQL integration passed 2/2 for tenant and scope references, finite quantities, immutable history, restricted roles, catalog validity, a two-client revision race, both permission revocation commit orders, and revoke-first catalog, membership and user changes. It also rejects higher-isolation mutation transactions that could retain stale authority. A separate opt-in disposable loopback writer integration passed for all seven scopes, multiple warning/stop thresholds, exact quantities, complete current read, concurrent optimistic revision conflict, disable projection, audit and tenant permission denial. The complete suite passed 438 tests (89 contracts, 49 database, 300 web), with one separate temporal race test skipped by its opt-in gate; root typecheck, lint and production build passed. No provider was called and no budget stop was tested.

The exact committed 0070 migration passed on hosted verification and production, with no pending SQL on either replay. Hosted restricted storage tests passed 2/2 when given a 30-second timeout for network latency; their first run exceeded the default five-second test timeout without an assertion result. [CI run 35817135292](https://github.com/mosnin/company-humans/actions/runs/35817135292) passed the committed SHA, including browser tests, built anonymous runtime, and signed usage over HTTP. Production deployment `dpl_CfgeMhcUPi6UcbRu3mqhHXBQ6Qzo` is READY, health reports `f30f84fc4ec49abda76fd15612c3d80b5cb0bfa0`, anonymous organizations, unsigned usage POST and budget GET return 401, and Apps redirects to sign-in. These probes do not test an authenticated budget decision or provider enforcement.

The pure budget consumption comparison has focused tests for overlapping warning and hard-stop thresholds, parent/child scopes, ties, zero, quantities above JavaScript safe integer range, complete scope/window coverage, tenant/product/meter mismatches, mixed environment and aggregation, stale or future UTC periods, half-open period end, and context instant mismatch. It has no database or provider input. A fresh local 0001–0070 migration/seed passed 448 tests (99 contracts, 49 database, 300 web) with root typecheck, lint and build; the separate 0069 temporal race test remained opt-in. An independent read-only review accepted the bounded contract. Capability quantities and atomic aggregate snapshot provenance remain unverified, so this test does not establish a spend decision or provider stop.

[CI run 35818818443](https://github.com/mosnin/company-humans/actions/runs/35818818443) passed the exact pure-contract commit, including browser, built anonymous runtime and signed usage HTTP checks. No migration was needed. Production deployment `dpl_9G5SwwjznnSSJLm2dqLfcj2kLZwr` is READY; health reports `1a34d1cfc02525067ba2f43320b27851b824c389`. Anonymous organizations, unsigned usage POST and budget GET return 401; Apps redirects to sign-in. No authenticated budget evaluation or provider stop was exercised.

### Signed reported capability attribution — 2026-09-23

On a fresh disposable 0001–0071 PostgreSQL migration and seed, 454 automated tests passed (101 contracts, 50 database, 303 web); the separate temporal concurrency fixture remained opt-in. Contract tests cover legacy omission, malformed keys and signature tampering. Restricted database tests cover generated projection, unknown historical rows, direct rewrite denial, tenant RLS, private envelope/signature columns, exact retry versus changed-key conflict, and capability-specific sum, maximum and last reads. Organization totals still combine capabilities for all three aggregation modes. Root typecheck, lint and production build passed after the browser fixture gained the nullable field. The Vite browser suite passed 96 desktop/mobile fixture checks. A direct invocation of the separate anonymous runtime suite lacked its required running Next and local Convex services and returned connection refused; it is run with those services by the CI harness, so no runtime acceptance is claimed from that direct attempt. These tests establish reported attribution only, not catalog authorization or provider hard stops.

[CI run 35820556325](https://github.com/mosnin/company-humans/actions/runs/35820556325) passed at commit `e60ef10916980de55444b407186bc222b4e1546c`, including fresh PostgreSQL migration/seed, tests, browser checks, built anonymous Convex/Next runtime and signed usage HTTP. Hosted verification replayed the committed 0071 migration with no pending SQL and passed the three focused restricted usage tests. Production preflight found zero invalid existing envelope capability values, then applied the exact committed 0071 migration; both hosted databases replay with no pending SQL. Production deployment `dpl_2m4qTgtUAcoHStbC7kk3TkFmnMmh` is READY at https://company-humans.vercel.app, with health reporting that exact commit SHA. Anonymous organizations, unsigned usage POST, budget GET and capability usage GET return 401; Apps redirects to sign-in. No authenticated capability usage view, catalog binding or provider enforcement was exercised on production.

### Advisory budget consumption snapshot — 2026-09-23

Migration 0072 and its two opt-in disposable-database integration tests cover a no-login, read-only role; dual `budgets.manage`/`usage.read.all` checks; tenant and private-column denial; exact current policy revisions; current operation bindings; zero and reported usage across all seven applicable scopes; unknown attribution failure; and refusal to authorize usage. An independent review found and prompted fixes to the snapshot instant and member-to-team binding before accepting the slice. The public reader now owns the repeatable-read transaction and first snapshot statement. A fresh disposable 0001–0072 migration and seed passed 455 automated tests (101 contracts, 51 database, 303 web), root typecheck, lint, and production build. The existing desktop/mobile browser suite passed 96 checks. A separate full rerun briefly hit local `ENOSPC` during Vitest temporary-file creation; after removing only regenerated build caches and an inactive temporary PostgreSQL data directory, the final full gate passed. No live provider stop or authenticated production budget decision was exercised.

Commit `9be9488c62f681a84af15ebf1455105ceb79a786` passed [CI run 35823528308](https://github.com/mosnin/company-humans/actions/runs/35823528308), including fresh migration/seed, tests, browser checks, built anonymous runtime and signed usage HTTP. Hosted verification applied the committed 0072 migration and replayed with no pending SQL. Live role checks confirmed no login, superuser, bypass-RLS or inherited privileges; the projected capability column is selectable while the signed envelope and writes are denied. Production applied the same committed migration and both hosted replays show no pending SQL. Vercel deployment `dpl_FR7UaQ9Jx2d7i5pYY349hiqUnoXB` is READY at https://company-humans.vercel.app; health reports the exact SHA. Anonymous organization, unsigned usage POST, budget and capability-usage requests return 401; Apps redirects to sign-in. These probes do not establish authenticated budget decisions or provider enforcement.

The snapshot integration now also registers `maximum` and `last` meters on a disposable PostgreSQL database. It verifies exact zero for empty windows, a peak of 9.5 across three values, and a deterministic latest value of 7 when occurrence and report timestamps tie and event ID breaks the tie. A focused fresh 0001–0072 migration/seed, snapshot test, database typecheck and lint passed. The test fixture pins the older event to the current UTC month start to avoid a calendar-boundary failure. This is SQL aggregation evidence only; it does not define reservation semantics for non-sum meters.

The test-only commit `12ae1fd6c7c2f89dbc22006a5bb5cb4c99fe1f9e` passed [CI run 35824299026](https://github.com/mosnin/company-humans/actions/runs/35824299026), including full migration/seed, automated and browser suites, built anonymous runtime, and signed usage HTTP checks. No migration or runtime behavior changed in that commit.

Migration 0073 narrows the snapshot role's column grants without changing runtime behavior. On a disposable database, 0001–0073 migrated and seeded, both opt-in budget snapshot integrations passed, and privilege probes confirmed six unused fields are denied while required catalog, member, team and mapping fields remain readable. The fresh full gate passed 455 automated tests (101 contracts, 51 database, 303 web), root typecheck, lint and production build. Hosted migration and CI remain to be recorded for the 0073 commit.

Commit `51a1bddcc141441bca5759e903657801060117b5` passed [CI run 35824716742](https://github.com/mosnin/company-humans/actions/runs/35824716742), including browser, built anonymous runtime and signed usage HTTP checks. Verification and production applied the exact committed 0073 migration and replayed with no pending SQL. Live grant probes on both show the six columns denied and the required projected capability key readable. Vercel deployment `dpl_HoWmBiSr6zecc1TbjYtQ4GxFh48z` is READY at https://company-humans.vercel.app; health returns that SHA. Anonymous organizations, unsigned usage POST, budget and capability usage return 401, and Apps redirects to sign-in. No authenticated budget admission or provider stop was exercised.

### Pre-cost reservation contract — 2026-09-23

The contract tests cover a second client key for the same source operation, changed source/payload conflicts, test/production separation, exact accepted plus outstanding plus requested quantity across policy thresholds, missing or mismatched outstanding scope/window rows, false authorization flags, expiry ordering, source-bound settlement evidence and late reconciliation after release/expiry. A red test pass first exposed missing source/projection/evidence behavior. The final repository run passed 111 contract tests, one non-opt-in database test (52 database integration tests skipped by their opt-in guards), and 303 web tests, plus root typecheck, lint and production build. No database migration, provider operation, real reservation or signed usage-to-operation binding was exercised by this slice.

The pushed contract commit `d1adab289208513c988abe25062c594c06c19e48` passed [CI run 35826488216](https://github.com/mosnin/company-humans/actions/runs/35826488216), including the existing migration, browser and anonymous runtime gates. This does not establish reservation persistence or provider enforcement.

### Signed usage operation provenance — 2026-09-23

Focused contract checks prove legacy signatures still verify, an optional operation ID survives signing, and tampering or removing it invalidates the signature. On a fresh disposable PostgreSQL database migrated through 0074 and seeded, focused ingestion checks prove null legacy projection, exact replay, changed signed operation ID conflict, cross-tenant denial, multiple events for one operation, invalid direct projection input rejection and immutable envelope/column behavior. The root gate passed 112 contract tests, one non-opt-in database test (52 opt-in database tests skipped), 303 web tests, typecheck, lint and production build. Provider operation identity and reservation settlement were not verified.

The independent 0074 review found only stale statements saying signed usage lacked an operation ID; those were corrected in the contract comment and billing/security/status docs. It confirmed the signing, legacy compatibility, projection, multi-event operation and tenant behavior. After adding migration-local lock and statement timeouts, a second fresh disposable 0001–0074 migrate/seed passed, the focused usage-ingestion and migration integrations passed 2/2, and replay had no pending migrations. Hosted table-size and lock checks remain a release gate.
