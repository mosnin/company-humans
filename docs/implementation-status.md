...
- A scaffold sweep removed ten unreferenced inherited UI exports and corrected the migration assessment to reflect the owner's Convex Auth choice; the Company OS product catalog remains intentional.
...
| 04 Human workspace | In progress | Work assignments and reports, role-aware Work navigation, bounded admin Work control, and a contributor Home action summary are released. Remaining native modules, richer Home content, broader manager workflows and authenticated production proof remain. Apps assignment visibility does not launch real sponsored access. |
...
### CH-22 pure budget consumption comparison — 2026-09-23

A read-only contract compares every applicable stored threshold against a complete caller-supplied quantity for its exact scope and UTC window. It preserves each action separately, uses six-decimal BigInt arithmetic, rejects missing or duplicate quantities, and binds all rows to an expected production/test environment, registered meter aggregation and evaluation instant inside the current day/week/month period. It returns below/reached/exceeded, not an access decision. Fresh local migration/seed and 448 tests passed (99 contracts, 49 database, 300 web), along with root typecheck, lint and production build. Independent review accepted the pure contract. There is no restricted database composition path yet, and current signed usage cannot supply capability attribution. No reservation, entitlement decision, product hard stop or real Scalar enforcement is claimed. CH-22 and Phase 03 remain in progress.

Commit `1a34d1cfc02525067ba2f43320b27851b824c389` passed [CI run 35818818443](https://github.com/mosnin/company-humans/actions/runs/35818818443), including browser, built anonymous runtime and signed usage HTTP checks. No migration was added. Vercel deployment `dpl_9G5SwwjznnSSJLm2dqLfcj2kLZwr` is READY at https://company-humans.vercel.app and live health reports the exact commit SHA. Anonymous organizations, unsigned usage POST and budget GET return 401; Apps redirects to sign-in. The pure evaluator has no live API caller and no provider enforcement evidence.

### CH-22 reported capability usage attribution — 2026-09-23

The signed V1 usage payload now carries an optional canonical capability key. Migration 0071 projects that key from the immutable envelope into a read-only generated column, retaining null for legacy or explicitly unknown attribution. A separate capability aggregation breakdown and exact filter expose the reported dimension without splitting existing organization/product/instance/team/member totals. The authenticated usage API accepts the same breakdown and filter. A fresh disposable 0001–0071 migration/seed passed 454 tests, root typecheck, lint, production build and 96 desktop/mobile browser checks. This is reported attribution only. The key is not checked against historical catalog or provider operations; no trusted budget database snapshot, authorization, reservation or hard stop exists. CH-22 and Phase 03 remain in progress.

Commit `e60ef10916980de55444b407186bc222b4e1546c` passed [CI run 35820556325](https://github.com/mosnin/company-humans/actions/runs/35820556325), including browser, built anonymous runtime and signed usage HTTP gates. Hosted verification passed three focused restricted usage tests. Production applied the same committed 0071 migration and both hosted replays report no pending SQL. Vercel deployment `dpl_2m4qTgtUAcoHStbC7kk3TkFmnMmh` is READY at https://company-humans.vercel.app; health returns the exact SHA. Anonymous organizations, unsigned usage POST, budget GET and capability usage GET deny with 401; Apps redirects to sign-in. Reported capability attribution has no provider operation proof or active budget decision.

### CH-22 advisory budget consumption snapshot — 2026-09-23

Migration 0072 adds a no-login, column-limited role with tenant RLS requiring both budget-management and organization usage-read authority. A dedicated server reader checks current policy, catalog, meter, instance, member, and team bindings, and compares accepted/released usage for every applicable scope and UTC window within one repeatable-read, read-only transaction. Missing policy coverage or relevant unknown member, team or capability attribution fails closed. The diagnostic always reports `authorizesUsage=false` and `providerEnforcementConfirmed=false`; no application endpoint calls it. Independent review identified two issues and accepted the corrected reader: it now owns the transaction/snapshot instant and requires a live member-team assignment for team scope. Fresh local 0001–0072 migration/seed, 455 automated tests, root typecheck, lint, production build and 96 desktop/mobile browser checks passed. CH-22 and Phase 03 remain in progress because this is not a capacity reservation, entitlement decision, priced cross-product budget, or demonstrated Scalar hard stop.

Commit `9be9488c62f681a84af15ebf1455105ceb79a786` passed [CI run 35823528308](https://github.com/mosnin/company-humans/actions/runs/35823528308), including browser, built anonymous runtime and signed usage HTTP gates. Hosted verification and production applied the same committed 0072 migration; both replay with no pending SQL. Production deployment `dpl_FR7UaQ9Jx2d7i5pYY349hiqUnoXB` is READY at https://company-humans.vercel.app, and health returns that exact SHA. Anonymous organizations, unsigned usage POST, budget GET and capability usage GET deny with 401; Apps redirects to sign-in. No authenticated budget comparison, provider operation binding, reservation or hard stop was exercised in production.

The advisory snapshot's PostgreSQL integration now covers `maximum` and deterministic `last` meters, including zero and timestamp-tie ordering, alongside existing `sum` coverage. Fresh disposable migration/seed, focused test, database typecheck and lint passed. Test-only commit `12ae1fd6c7c2f89dbc22006a5bb5cb4c99fe1f9e` passed [CI run 35824299026](https://github.com/mosnin/company-humans/actions/runs/35824299026). This extends test evidence only; production runtime logic and the CH-22 acceptance state are unchanged.

### CH-22 snapshot role grant reduction — 2026-09-23

Migration 0073 revokes six columns that the advisory snapshot reader does not use. The reader and restricted-role regression now prove those fields are unavailable while its exact policy, catalog, member, team and usage projection still works. Fresh disposable 0001–0073 migration/seed and 455 automated tests passed, along with root typecheck, lint and production build. This is least-privilege maintenance, not a new budget decision or provider stop. CH-22 and Phase 03 remain in progress.

Commit `51a1bddcc141441bca5759e903657801060117b5` passed [CI run 35824716742](https://github.com/mosnin/company-humans/actions/runs/35824716742), including browser, built anonymous runtime and signed usage HTTP gates. Verification and production applied the exact committed 0073 migration; both replay with no pending SQL. Live grants deny all six removed columns and retain the required projected capability key. Production deployment `dpl_HoWmBiSr6zecc1TbjYtQ4GxFh48z` is READY at https://company-humans.vercel.app, and health reports the exact SHA. Anonymous organizations, unsigned usage POST, budgets and capability usage deny with 401; Apps redirects to sign-in. Provider admission and hard-stop behavior remain unverified.

### CH-22 pre-cost reservation contract — 2026-09-23

A provider-neutral V1 contract now binds source operation identity separately from client retry keys, projects exact accepted/released plus outstanding plus requested `sum` usage across all applicable policy thresholds, and models append-only capacity transitions and late usage reconciliation after settlement, release or expiry. It preserves each action and emits only non-authorizing diagnostics. The first independent review found duplicate-operation, projected-headroom, settlement and late-usage gaps; the revised contract addresses those at the shape and pure-comparison level. The full local test/typecheck/lint/build gate passed (111 contract, one non-opt-in database, 303 web tests). At this contract-only checkpoint, no reservation table or writer, live Scalar enforcement, manager approval flow, or verified event-to-reservation settlement binding existed; CH-22 and Phase 03 remain in progress.

Commit `d1adab289208513c988abe25062c594c06c19e48` passed [CI run 35826488216](https://github.com/mosnin/company-humans/actions/runs/35826488216). This verifies the pushed contract and existing CI gates; no provider operation or production reservation was exercised.

### CH-22 signed operation provenance — 2026-09-23

The V1 signed event source now permits an optional operation ID, and migration 0074 projects it from immutable stored envelopes. Legacy events remain null; one operation may produce multiple signed usage events. Fresh disposable 0001–0074 migration and seed plus focused ingestion/contract tests passed. The full local suite passed 112 contract tests, one non-opt-in database test (52 opt-in tests skipped), and 303 web tests, with root typecheck, lint and production build. This records signer-reported identity only; reservation settlement, provider legitimacy, capacity locks and live Scalar hard stops remain unverified. CH-22 and Phase 03 remain in progress.

Independent review of 0074 accepted the code and SQL behavior after stale documentation was corrected. The migration now fails atomically on lock contention or a rewrite exceeding its local timeout; the exact revised SQL passed a fresh disposable migrate/seed, 2 focused integrations and no-pending replay. Hosted deployment and real provider operation verification remain outstanding.

Commit `1cdf84be52d0af3919a2c5bf47695514858a5b86` passed [CI run 35827864222](https://github.com/mosnin/company-humans/actions/runs/35827864222). Exact 0074 migrated and replayed clean on hosted verification and production after zero-row/lock preflight; verification signed ingestion passed. Production deployment `dpl_4jyNryLdTqKQZk13eNLQVxT8Vv4D` is READY at https://company-humans.vercel.app with health at that SHA. Anonymous organizations, usage and budgets deny with 401; Apps redirects to sign-in. There is still no authenticated live provider usage, reservation settlement or hard stop.

### CH-22 inert reservation storage — 2026-09-23

Migration 0075 defines source-operation and transport-key uniqueness, typed tenant/product/meter references, positive `sum` quantities, database expiry, immutable intent and ordered append-only ledger. A second event can reconcile after settled, released or expired capacity without changing terminal state; event IDs are unique across the ledger. A database finite-quantity check closes direct NaN usage insertion. Forced RLS has zero policies and runtime grants, so no application can read or write these tables. The focused fresh disposable migrate/seed/integration passed, as did 112 contract tests, one non-opt-in database test, 303 web tests, typecheck, lint and build. The administrative event fixture proves relational structure only. No capacity hold, provider grant, verified settlement, billing, or Scalar stop is claimed; CH-22 and Phase 03 remain in progress.

Commit `fe14ba0096b11c78e6c78e7aabee6c0e058ae5dc` passed [CI run 35829971331](https://github.com/mosnin/company-humans/actions/runs/35829971331). Verification and production applied exact 0075 and replayed without pending SQL; both have forced RLS, no policies or grants to other roles, zero reservation rows and zero nonfinite usage rows. Production deployment `dpl_BCoiKE3jLpM8quLjf5EHot5mcfXu` is READY at https://company-humans.vercel.app and health reports that SHA. Anonymous organizations, usage and budgets deny with 401; Apps redirects to sign-in. Authenticated, provider and settlement acceptance remains open.

### CH-22 signed stored-usage evidence — 2026-09-23

A read-only verifier now rechecks a persisted usage HMAC over the exact raw body under a caller-supplied active signing authority and compares the signed body with the stored tenant, product, instance, source operation, human actor, attribution, meter, quantity and timestamps. The shared event and audit signature helpers reject unsigned nested fields before normalization. The reader fails when the exact sum meter or required operation ID is missing, and labels all results as unable to authorize usage or settlement. Local pure and disposable database tests prove signed ingestion is accepted as evidence while a fabricated accepted row is rejected. The repository test/typecheck/lint/build gate passed. The current ingestion role can still insert directly, historical/revoked keys fail closed, and quarantine release presence is not independent authorization. No reservation writer, capacity hold, bill or provider stop was added; CH-22 and Phase 03 remain in progress.

The initial independent review found and reproduced an unsigned nested-field bug; exact raw-body HMAC verification and strict nested schemas closed it, and re-review accepted the bounded change. Commit `e67e1acb7de3a4c9a1143d1b7481e9152bd08a99` passed [CI run 35832198897](https://github.com/mosnin/company-humans/actions/runs/35832198897). Production deployment `dpl_CgYs6KoH4CgNXcD5Dv574py1XmHd` is READY at https://company-humans.vercel.app, health returns that SHA, anonymous organizations/usage/budgets deny with 401, and Apps redirects to sign-in. No production signing key or provider use was demonstrated; Phase 03 remains in progress.

### CH-22 staged database signing boundary — 2026-09-23

Migration 0076 prepares an empty private scoped key registry and an owner-only HMAC ingestion function. No runtime role can call it or read keys, the existing API still inserts through the old ingest role, and direct database INSERT has not been revoked. The migration rejects unsafe preexisting writer roles and a nonpublic `pgcrypto` installation. A fresh disposable 0001–0076 migration/seed/replay and three focused HMAC, replay, scope, tamper, conflict, quarantine, revocation and privilege tests passed; a separate non-superuser `CREATEROLE` PostgreSQL 18 cluster proved the migration-owner ADMIN edge grants no `SET` or inheritance. The full local test/typecheck/lint/build gate passed. Strict nested payload equivalence, duplicate identity ambiguity, concurrent retries and timing behavior remain grant blockers. This stage does not close Phase 03 or allow reservation settlement, billing or provider use.

Commit `4217cada14abf7744e91d280e36bb7b4c6504e73` passed [CI run 35834745353](https://github.com/mosnin/company-humans/actions/runs/35834745353). Verification and production applied its exact 0076 SQL and replayed with no pending migration. Read-only checks in both databases found zero signing keys, revocations and usage rows; `pgcrypto` in `public`; forced RLS on both key tables; a no-login, noninheriting writer with no elevated membership edge; and no legacy runtime EXECUTE on the new function. The legacy ingest role still has direct INSERT by design at this inert checkpoint. Production deployment `dpl_8Gqco7wR6LHjN3nxZnrfQDorb8xP` is READY at https://company-humans.vercel.app; health returns the exact commit SHA. Anonymous organizations, unsigned usage and budgets return 401, and Apps redirects to sign-in. An independent parity review identified signed noncanonical JSON, nested schema/type, timestamp, duplicate identity and dual key-registry gaps; no runtime grant or financial trust claim follows from this release.

### Phase 02 admin member readiness detail — 2026-09-23

An administrator can open a mapped member's read-only activation check from Applications. The page explains current identity, permission, provider binding, capability delivery, denial receipt, limit delivery and meter-enforcement reasons, with links to the corresponding settings and health views. Both `applications.manage` and `budgets.manage` are checked before the database read; the reader also requires both and binds the mapping to the selected organization and product instance. A missing or foreign mapping returns no subject. This closes a visibility gap only: the diagnostic always reports `ready=false`, offers no launch or activation, and does not verify Scalar sponsorship or a provider hard stop. CH-15, CH-18, CH-19 and Phase 02 remain in progress.

Commit `91d6a36abbad9d98c897ba598b7c3c50590badbe` passed [CI run 35837010998](https://github.com/mosnin/company-humans/actions/runs/35837010998). Production deployment `dpl_AjS4ddQjCCerbcTjB9MkGKCizBZw` is READY; live health reports that exact revision. Anonymous organizations, unsigned usage and budgets return 401, and the new readiness route redirects to sign-in. These are deployment and denial checks, not an authenticated administrator or Scalar acceptance scenario.

### Phase 04 native module settings foundation — 2026-09-23

Migration 0077 and a server-only database API store tenant-scoped revisions for the eight first-party modules named in Notion 07. Work defaults on; the other seven default off. Organization administrators may append a setting revision; active members can read effective settings. The module table does not grant connected product access. At this foundation release, no workspace page, navigation gate or module administration UI consumed it. The later 0078 Work slice now consumes the Work setting; CH-27 and Phase 04 remain in progress. Local fresh migration, restricted-role integration, full suite, typecheck, lint and build passed. The independent reviewer accepted this bounded database foundation and kept CH-27 open.

Commit `1c4a848234c60da050cab03f8ce2b816f11a5f80` passed [CI run 35839208513](https://github.com/mosnin/company-humans/actions/runs/35839208513), including fresh migration/seed, browser and built anonymous runtime checks. The exact committed 0077 migration applied on hosted verification and production and replayed with no pending SQL in both. Read-only probes found forced RLS, three intended policies, app access to `enabled` but not actor identity, no app INSERT, no service UPDATE and zero visible module revisions. Production deployment `dpl_9x87kDpQLDtpXvE2eRTcCCtzALyV` is READY; live health reports the exact SHA. Anonymous organizations and budgets return 401, and Apps redirects to sign-in. An authenticated organization module write/read or rendered native Work route has not been exercised in production.

### CH-29 native Human Work first slice — 2026-09-23

Migration 0078 introduces tenant-scoped human assignments and immutable assignee completion reports with transactional audit, forced RLS, current team and organization permission checks, and serialized Work-module shutdown. The Work page lets an authorized manager assign an active member and lets an active assignee report an outcome with required evidence. The navigation and page respect the native Work setting. This is manager-originated obligation and assignee-reported completion; there is no outcome verification, CRM link, workflow source, cancellation, or Operate agent task import. The completion label is “Reported complete.” CH-29 and Phase 04 remain in progress until the full Notion acceptance criteria are demonstrated.

Local evidence at commit `88ae1c972f1cc2b5578209bbd1b6b62c6282c256`: fresh PostgreSQL 18 migrations 0001–0078 and replay; restricted-role Human Work integration over TCP, including cross-tenant denial, manager/team boundaries, duplicate report races, audit rollback, and a deterministic Work-disable race; 114 contract tests, 352 web tests, package type checks, lint, production build, and four desktop/mobile browser fixture checks passed. A built local server redirected anonymous Work to sign-in and rejected anonymous create/complete requests with 401. The browser checks mock auth and data. Independent review accepted the bounded slice after a custom-grant manager-view defect was fixed and covered. [CI run 35842595718](https://github.com/mosnin/company-humans/actions/runs/35842595718) passed at the exact SHA.

Hosted verification and production applied the exact committed 0078 SQL and replayed with no pending migration. Read-only probes found forced RLS on both tables, six intended policies, app INSERT denied, service UPDATE denied, a volatile module-state check, and zero assignment/completion rows. Production deployment `dpl_AMKaiTWVtMTjgGdRkd8mDe1qiU4h` is READY at https://company-humans.vercel.app; live health returns the exact SHA, anonymous Work redirects to sign-in, and anonymous create/complete requests return 401. This is deployment and denial evidence, not an authenticated manager/contributor or persisted production assignment. The source Company OS repository remains untouched. CH-29 and Phase 04 remain in progress.

### CH-27 administrator native module controls — 2026-09-23
...
### CH-28 contributor Home first action slice — 2026-09-23

Home now reads up to three open own Human Work assignments through the restricted runtime role when Work is enabled and the member has `assignments.read.own`. It distinguishes a disabled module, unavailable module state, an empty queue and an assignment read failure. Its Apps link uses the existing own-member reader and cautious status copy; historical removed mappings cannot inflate a displayed “assigned apps” count. It shows no fabricated CRM, earnings, target or leaderboard figures. The independent reviewer accepted this bounded slice after the misleading app count was removed. Full CH-28 and Phase 04 remain in progress until leads, pipeline, earned/pending/paid, targets, team signal and relevant health have authoritative sources and verified views.

Local candidate evidence: 114 contract tests, 377 web tests, typecheck, lint, production build and two desktop/mobile browser fixture checks passed. The mobile Home screenshot was inspected at 390px with no overflow. Browser data is mocked; authenticated production Home has not been exercised. No new migration or provider integration is part of this slice.

Commit `a7aee7d33f7bb5ece4e57f5a58202e127b9fbb10` passed [CI run 35845115189](https://github.com/mosnin/company-humans/actions/runs/35845115189). Production deployment `dpl_Ba4bswi85XdL6vC5K8Pa9Nywzy9w` is READY at https://company-humans.vercel.app; live health returns that exact SHA. Anonymous Home, Work and Workspace settings redirect to sign-in. No authenticated production assignment or app summary was rendered, so CH-28 remains in progress.
