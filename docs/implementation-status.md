...
Migration 0077 and a server-only database API store tenant-scoped revisions for the eight first-party modules named in Notion 07. Work defaults on; the other seven default off. Organization administrators may append a setting revision; active members can read effective settings. The module table does not grant connected product access. No workspace page, navigation gate or module administration UI consumes it yet, so CH-27 and Phase 04 remain in progress. Local fresh migration, restricted-role integration, full suite, typecheck, lint and build passed. The independent reviewer accepted this bounded database foundation and kept CH-27 open.
...
Local evidence on the uncommitted candidate: fresh PostgreSQL 18 migrations 0001–0078 and replay; restricted-role Human Work integration over TCP, including cross-tenant denial, manager/team boundaries, duplicate report races, audit rollback, and a deterministic Work-disable race; 114 contract tests, 352 web tests, package type checks, lint, production build, and four desktop/mobile browser fixture checks passed. A built local server redirected anonymous Work to sign-in and rejected anonymous create/complete requests with 401. The browser checks mock auth and data. Independent review accepted the bounded slice after a custom-grant manager-view defect was fixed and covered. CI, hosted migration, production release, and authenticated user testing are still pending. The source Company OS repository remains untouched.
