...
A provider-neutral V1 contract now binds source operation identity separately from client retry keys, projects exact accepted/released plus outstanding plus requested `sum` usage across all applicable policy thresholds, and models append-only capacity transitions and late usage reconciliation after release/expiry. It preserves each action and emits only non-authorizing diagnostics. The first independent review found duplicate-operation, projected-headroom, settlement and late-usage gaps; the revised contract addresses those at the shape and pure-comparison level. The full local test/typecheck/lint/build gate passed (111 contract, one non-opt-in database, 303 web tests). No reservation table or writer, live Scalar enforcement, manager approval flow, or verified event-to-reservation settlement binding exists; CH-22 and Phase 03 remain in progress.
