...
## Native Human Work first slice

The first Work route is backed by Company Human assignments rather than Operate agent tasks. A manager creates a human obligation for an active member; the assignee can read it and submit one immutable completion report with an outcome and evidence. Managers can inspect assignments only within their authorized team or organization scope. Work navigation appears only when the native module is enabled and the member has assignment-read permission; the page and database repeat those checks. This slice supports manager-originated assignments only. Workflow/product sources, CRM links, blocked and cancellation states, outcome verification, and Operate handoff remain separate work.

The contributor Home now reads the first three open own assignments through the same tenant-scoped Work reader, conditioned on the Work module and `assignments.read.own`. It links to the full Work queue. App status uses the existing own-member application reader and points to Apps; its records do not assert provider launch access. The Home omits leads, pipeline, earnings, targets, leaderboard, wins, and announcements until their authoritative modules exist.
