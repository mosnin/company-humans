...
The same suspended binding checks the target member's current tenant role against product.use and every catalog-required permission. If a grant disappeared during the provider call, binding retains the suspended identity and queues an authorization-origin fenced denial. A bound member whose grant is later revoked receives the same durable denial stream. Draft entries are ineligible for bootstrap and positive member limits; zero hard stops remain available. No live provider has demonstrated these transitions yet.
