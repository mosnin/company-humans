...
GET /api/organizations/[organizationId]/usage requires an authenticated workspace user and explicit environment, from, until and breakdown. Optional productInstanceId, membershipId and teamId narrow the RLS-visible set. The response scope is permitted-usage; consumers must not label a contributor subset as organization-wide consumption. Quantities and event counts are decimal strings.

POST /api/organizations/[organizationId]/usage/[eventId]/release takes a strict JSON reason. An authorized billing administrator can release only a quarantined event whose exact meter is registered. released=false denotes an existing receipt, preserving its first actor and reason. This endpoint does not alter the signed product event.

Fenced member activation uses ProductMemberAccessAdapterV1. Providers must enforce revisions at actual execution, bind exact identities/policies, expose durable operation and current-state readback, and route denials through the same fence. The reusable member-access-conformance package export tests an admitted pending grant released after newer denial, policy content/coverage mismatches, reconciliation and removal. Real Scalar implementation and activation orchestration are still required.

Fenced denial dispatch now replaces legacy suspend/remove dispatch. Registrations must implement ProductMemberAccessAdapterV1. The worker reconciles stable operation identity, verifies exact command receipts and current readback, then rechecks current provider binding/fence before recording success. Missing or changed bindings produce provider_binding_reconciliation_required without a provider call. Automatic late-bootstrap recovery and all grant/activation execution remain open.

Canonical identity tombstones now produce durable fenced product denial intent, including historical tombstone reconciliation. This does not implement a Convex deletion event transport or prove remote session revocation. Profile login sync remains active-only; a verified deletion delivery path and actual provider execution remain necessary. Global suspension is not silently substituted for deletion.
...
An inactive sponsoring organization creates provider-bound member suspension intent through the existing fenced denial journal. Delivery remains asynchronous and must be confirmed by an exact provider readback. Restoring the organization does not grant or resume access. No live provider accepted this transition yet.

Product catalog retirement or changes to capabilities, meters, required permissions, adapter version, provisioning modes, billing behavior, member operations or connection requirements create the same fenced denial intent for already bound members. Display-only edits do not. The integration must confirm the exact current revision; no Scalar or other live provider has accepted a catalog-triggered denial yet.

Suspended member creation records the product access-contract revision when its command is created. If the catalog changes before its provider identity is bound, the binding is kept suspended and a single catalog-origin denial is queued for that mapping. A draft product refuses new binding. This preserves provider identity for reconciliation while preventing an older creation command from silently satisfying a newer catalog contract. No live provider has demonstrated this path yet.

The same suspended binding checks the target member's current tenant role against product.use and every catalog-required permission. If a grant disappears after the worker's eligibility read but before binding, binding retains the suspended identity and queues an authorization-origin fenced denial. If the worker observes loss when the receipt arrives, it supersedes the command without binding. A bound member whose grant is later revoked receives the durable denial stream. Draft entries are ineligible for bootstrap and positive member limits; zero hard stops remain available. No live provider has demonstrated these transitions yet.

Bootstrap now refuses a provider call when the target is already blocked or missing a catalog requirement. If eligibility disappears during a provider call, the returned provider reference is retained in bootstrap history and the command is superseded without canonical binding. A future reconciliation worker must find and suspend or remove that remote identity; the recorded reference is not proof of cleanup. No real provider has been exercised.

New member requests now require a ready catalog entry and the target's current role grants before writing a pending provision command. The database insert policy enforces this for direct service SQL as well as the application service. A request does not call Scalar or any other provider and cannot be interpreted as sponsored access. Already blocked or disabled mappings stay in the reconciliation path.

The activation readiness diagnostic can now identify which local suspended-member, capability and limit receipts are absent or stale. Even a locally complete fixture returns `meter_semantics_unverified`, since no adapter has proved an exact meter/unit/window hard stop. It sends no Scalar request, grants no access, and does not clear policy blocks. Scalar's real control API and provider enforcement remain Phase 02/03 acceptance gates.

The shared product meter enforcement declaration schema describes the fields a future provider registration must attest: claimed variable-cost meter keys pinned to definition versions, units, aggregation, finite UTC windows, organization/member scopes and pre-cost hard stops. It checks shape only. No declaration is persisted or compared with registered meters, existing V1 limit receipts carry no meter version, no provider verification is recorded, and no product is activated by parsing it. The inspected Scalar main source at `f773ee94e32406c93b6d51dd408aa40e51938975` still lacks the required tenant-bound control API, suspended member lifecycle and atomic spend boundary; see [Scalar control assessment](scalar-control-assessment.md).

An additive V2 limit adapter extension can now express and read back the exact meter version as part of a finite hard-stop policy. V1 adapter registrations remain compatible for their existing scope; a V1-only registration cannot satisfy V2. No V2 transport is registered, no saved limit or worker emits it, and Scalar has not demonstrated pre-cost enforcement. This is contract preparation, not enabled sponsored access.

The revision storage now distinguishes V1 from future V2 policies. V2 revisions cannot enter the V1 dispatch journal or fall back to the V1 worker, and the existing service writer cannot assert a V2 meter version. No V2 provider call or registry verification exists; Scalar access stays closed.

An operator-only comparison can now reject a meter declaration whose product revision, catalog meter key set or registered meter version/unit/aggregation has drifted. It permits historical registered versions to coexist. Even a locally compatible declaration never certifies a product's undisclosed cost surface, finite-window counter behavior or pre-cost hard stop. No Scalar declaration is registered as provider proof.

Signed usage reports may now include a canonical `capabilityKey` independent of the meter key. The ingestion signature and idempotency identity cover the field; migration 0071 stores a generated read projection from the original envelope. Omission remains null for older clients. The reported key does not prove that Scalar or another product bound the operation to an authorized capability, and no live adapter has yet supplied that proof. Product adapters must eventually emit the capability from their actual operation boundary, with a versioned historical catalog/access binding, before capability budgets can authorize spend.
