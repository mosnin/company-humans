...
Durable capability snapshot preparation is now available as a server-only database service. It derives the complete requested capability set for a bound suspended member, persists source provenance, deduplicates unchanged refreshes and emits atomic human audit. It does not call the provider, enqueue a staging worker, automatically refresh on every policy/catalog change or resume members. The next execution stage must refresh/revalidate snapshots and retain apply/readback receipts before any activation decision.
